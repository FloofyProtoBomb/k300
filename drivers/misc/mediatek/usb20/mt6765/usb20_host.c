...
static struct charger_consumer *otg_consumer;
#endif
#endif
#endif
...
#ifdef CONFIG_LGE_PM
	if (!otg_consumer) {
		otg_consumer = charger_manager_get_by_name(
				mtk_musb->controller, "usb20_host");
		if (!otg_consumer) {
			DBG(0, "get otg consumer device failed\n");
			return;
		}
	}
#endif
...
#if CONFIG_MTK_GAUGE_VERSION == 30
#ifdef CONFIG_LGE_PM
		charger_manager_enable_hz(otg_consumer, false);
#endif
...
#ifdef CONFIG_LGE_PM
		charger_manager_enable_hz(otg_consumer, true);
#endif
#else
