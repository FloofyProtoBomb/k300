...
#if defined(CONFIG_LGE_USB_FACTORY) && defined(CONFIG_LGE_PM_USB_ID)
...
void musb_start(struct musb *musb)
{
...

	{
		...
		#if defined(CONFIG_LGE_USB_FACTORY) && defined(CONFIG_LGE_PM_USB_ID)
		if (!lge_get_laf_mode() && (lge_get_board_cable() == LT_CABLE_130K
			|| get_meta_com_type() == META_USB_COM)) {
			/* Full Speed Only */
			val &= ~MUSB_POWER_HSENAB;
		}
		#endif
