...
int primary_get_shutdown_scenario(void)
{
	if((lge_get_mfts_mode() == STATUS_ON && get_auto_touch_test() != STATUS_ON))
		return MFTS_POWER_DOWN;

	if(pgc->plcm->params->gSetShutdown == true)
		return DEVICE_POWER_DOWN;

	return NORMAL_SUSPEND;
}
...
bool primary_get_chargerlogo_mode(void) {
  ...
  if(disp_lcm_check_chargerlogo_mode())
    ret = true;
	...
	int is_lcm_inited)
{
	...
	DISPERR("primary_display_init begin lcm=%s, inited=%d\n",
	...
	if (primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		...
		if(lcm_param != NULL){
			...
			if(lcm_param->lcm_seq_shutdown == LCM_SHUTDOWN_BEFORE_DSI_OFF){
				if(primary_get_shutdown_scenario() != NORMAL_SUSPEND || disp_lcm_check_chargerlogo_mode() == 1){
...
int primary_display_te_test(void)
{
	...
	if (ret >= 0)
		DISPERR("[display_test_result]==>Force On TE Open!(%d)(%d)\n", time_interval_max,time_interval);
	else
		DISPERR("[display_test_result]==>Force On TE Closed!(%d)(%d)\n", time_interval_max,time_interval);
