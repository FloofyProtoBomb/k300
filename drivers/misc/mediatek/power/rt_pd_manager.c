		...
		if ((pd_sink_voltage_new != pd_sink_voltage_old) ||
			...
			if (pd_sink_voltage_new) {
				...
				if (usb_compliance_mode) {
					pd_sink_current_new = pd_sink_current_new - 100;
					pr_info("%s set %dma\n", __func__, pd_sink_current_new);
					charger_manager_set_input_current_limit(chg_consumer,
						MAIN_CHARGER, pd_sink_current_new);
				}
#endif
#if CONFIG_MTK_GAUGE_VERSION == 30
