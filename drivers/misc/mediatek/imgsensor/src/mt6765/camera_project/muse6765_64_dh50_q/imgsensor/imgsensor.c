...
/*LGE_CHANGE_S, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
...
/*LGE_CHANGE_E, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
...
/*MDH50LM OTP bring up*/
mdh50lm_otp_data_t hi1336front_otp_data;
UINT8 imgsensor_model_name;
/*
char *otp_content[] = {"AWB_5100K", "LSC_5100K", "AWB_3100K"};
kal_uint16 mdh50lm_otp_read(otp_format_t *p, u16 i2c_addr, enum OTP_FORM otp_form, UINT8* data_c, u8 check)
{
    ...
    iReadRegI2C(pu_send_cmd , 2, (u8*)&group, 1, i2c_addr);
    printk("checksum group(%d) check_addr(0x%x)", group, p->check_addr);
    if (group != 0x01) {
		return -1;
    }
    pu_send_cmd[1] += 1;
    ...
    //read checksum
    pu_send_cmd[0] = (char)((p->check_addr) >> 8);
    pu_send_cmd[1] = (char)((p->check_addr) & 0xFF);
    iReadRegI2C(pu_send_cmd , 2, (u8*)&checksum_read, 2, i2c_addr);
    for (i = 0; i < p->size; i++)
		checksum += data[i];

	data_c[MDH50LM_OTP_SIZE - 2 - otp_form] = (((checksum % 0xFFFF)+check) == checksum_read);
	printk("mdh50lm_otp_read : (%d %s) group(%d) checksum(0x%04x) checksum_read(0x%x) check_ok2(%d)",
			otp_form, otp_content[otp_form], group, (checksum % 0xFFFF) + check, checksum_read, data_c[MDH50LM_OTP_SIZE - 2 - otp_form]);
#if 1
	for (i = 0; i < p->size - 5; i += 6)
		printk("mdh50lm_otp_read data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	while (i < p->size) {
		printk("mdh50lm_otp_read data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
	return 0;
...
//[LGE_UPDATE_S][bk.bae@lge.com][2020-02-13] revision check for rev11
		if(lge_get_board_revno() == HW_REV_1_1){
			pr_info("###camera custom i2c REV_1_1");
			...
			pr_info("###camera custom i2c REV_A");
			imgsensor_i2c_init(&psensor_inst->i2c_cfg,
				imgsensor_custom_config_reva[psensor->inst.sensor_idx].i2c_dev);
		} else {
			pr_info("###camera custom i2c other rev");
			imgsensor_i2c_init(&psensor_inst->i2c_cfg,
				imgsensor_custom_config_other_rev[psensor->inst.sensor_idx].i2c_dev);
		}
...
static ssize_t show_LGCameraSensorName(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //if(lge_camera_hwinfo == 3) hardcoding for DH50
...
{
    struct device*  camera_sensor_name_dev;/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
