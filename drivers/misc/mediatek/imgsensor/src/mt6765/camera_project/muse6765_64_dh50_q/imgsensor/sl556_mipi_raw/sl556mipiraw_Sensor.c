...
#define per_frame 1
extern char wide_sensor_name[20];/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/
...
{
LOG_DEBUG("[SL556W] init setting start\n");
#if MULTI_WRITE
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x0008, 0x0b00); //line length
    ...
    write_cmos_sensor(0x0006, 0x0823); //frame length
...
kal_uint16 addr_data_pair_capture_30fps_sl556[] = {
    0x0b0a, 0x8252,
    0x0f30, 0x6e25,
    0x0f32, 0x7067,
    0x004a, 0x0100,
    ...
    0x0a14, 0x0798, //formatter row output size
    0x003e, 0x0000,
    0x0074, 0x0821, //the integration time control
    0x0070, 0x0411,
    0x0804, 0x0200,
    0x0a04, 0x014a,
    0x090c, 0x0fdc,
...
{
LOG_DEBUG("[SL556W] capture setting start currentfps : %d\n", currefps);
#if MULTI_WRITE
	if (currefps == 300) {
	sl556_table_write_cmos_sensor(
...
//	write_cmos_sensor(0x0a00, 0x0000); // stream off
	if( currefps == 300 )
	{
        ...
        write_cmos_sensor(0x003e, 0x0000);
        write_cmos_sensor(0x0074, 0x0821); //the integration time control
	...
	else
	{
        ...
        write_cmos_sensor(0x0030, 0x1111);
        write_cmos_sensor(0x0032, 0x1111);
        write_cmos_sensor(0x0006, 0x1046); //frame length
        write_cmos_sensor(0x0116, 0x07b6);
        write_cmos_sensor(0x0a22, 0x0000);
        write_cmos_sensor(0x0a12, 0x0a20); //formatter column output size
        write_cmos_sensor(0x0a14, 0x0798); //formatter row output size
        write_cmos_sensor(0x003e, 0x0000);
        write_cmos_sensor(0x0074, 0x1042); //the integration time control
        write_cmos_sensor(0x0070, 0x0821);
        write_cmos_sensor(0x0804, 0x0200);
        write_cmos_sensor(0x0a04, 0x014a);
        write_cmos_sensor(0x090c, 0x0fdc);
        write_cmos_sensor(0x090e, 0x002d);
...
static void hs_video_setting(void)
{
    ...
    write_cmos_sensor(0x0006, 0x0208); //frame length
    write_cmos_sensor(0x0116, 0x07b6);
    ...
    write_cmos_sensor(0x003e, 0x0000);
    write_cmos_sensor(0x0074, 0x0206); //the integration time control
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					strcpy(wide_sensor_name, "sl556_cowell");/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/
...
{
    LOG_DEBUG("[SL556W] preview start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[SL556W] capture start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[SL556W] normal_video start\n");
