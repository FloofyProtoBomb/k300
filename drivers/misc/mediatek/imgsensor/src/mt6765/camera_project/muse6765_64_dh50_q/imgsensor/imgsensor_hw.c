...
#include <soc/mediatek/lge/board_lge.h>//[LGE_UPDATE][bk.bae@lge.com][2019-12-04] revision check for rev
...
//[LGE_UPDATE_S][bk.bae@lge.com][2020-02-13] revision check for rev11
		if(lge_get_board_revno() == HW_REV_1_1){
			pr_info("###camera custom pwr REV_1_1");
			pcust_pwr_cfg = imgsensor_custom_config;
		} else if(lge_get_board_revno() == HW_REV_A) {
			pr_info("###camera custom pwr REV_A");
			pcust_pwr_cfg = imgsensor_custom_config_reva;
		} else {
			pr_info("###camera custom pwr other rev");
			pcust_pwr_cfg = imgsensor_custom_config_other_rev;
		}
