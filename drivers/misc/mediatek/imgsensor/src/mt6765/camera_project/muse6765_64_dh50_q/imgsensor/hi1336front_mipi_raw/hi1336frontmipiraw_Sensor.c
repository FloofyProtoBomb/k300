...
#include "hi1336frontmipiraw_Sensor.h"

#define PFX "hi1336front_camera_sensor"
...
extern bool read_hi1336front_eeprom( kal_uint16 addr, BYTE *data, kal_uint32 size);
extern bool read_eeprom( kal_uint16 addr, BYTE * data, kal_uint32 size);
extern char front_sensor_name[20];/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/


#define MULTI_WRITE 1
static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct hi1336front_imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI1336FRONT_SENSOR_ID,
	...
	.pre = {
		...
		.framelength = 3331, 			//record different mode's framelength
    ...
    .custom1 = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 240,
		.mipi_pixel_rate = 403200000, //(1428M * 4 / 10 )
	},
    .custom2 = {
		.pclk = 600000000,
		.linelength = 7504,
...
static struct hi1336front_imgsensor_struct imgsensor = {
	.mirror = IMAGE_NORMAL,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
	...
	{ 4224, 3136,   0, 116, 4224, 2904,	  704,  484,  32, 2,  640,  480, 0, 0,  640,  480},		// hight speed video (640 x 480)
 	...
 	{ 4224, 3136,   0,   6, 4224, 3124,	 4224, 3124,  8,  2, 4208, 3120, 0, 0, 4208, 3120},     // custom1 (4208 x 3120)
	{ 4224, 3136,   0,   6, 4224, 3124,	 4224, 3124,  8,  2, 4208, 3120, 0, 0, 4208, 3120},     // custom2 (4208 x 3120)
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
	...
	.i4Crop = {{0,0},{8,8},{8,8},{0,0},{0,0},{8,8},{8,8},{0,0},{0,0},{0,0}},
};
#endif
...
static kal_uint16 hi1336front_table_write_cmos_sensor(
...
static kal_uint16 read_sensor_eeprom(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, HI1336FRONT_QTECH_EEPROM);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_hi1336front[] = {
	0x2000, 0x0021,
	0x2002, 0x04a5,
	0x2004, 0xb124,
	0x2006, 0xc09c,
	...
	0x0668, 0x4303, //0x4302, //blc_spare b15:8:blc nr weight threshold, b2:obp_range_scaler, b1:0:blc_dpc_mode
...
{
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_init_hi1336front,
		sizeof(addr_data_pair_init_hi1336front) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x0668, 0x4303); //blc_spare b15:8:blc nr weight threshold, b2:obp_range_scaler, b1:0:blc_dpc_mode
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_hi1336front[] = {
	...
	0x0b04, 0x037c, //isp enable 0x037e
	//0x027e, 0x0100, //tg enable
};
#endif
...
static void preview_setting(void)
{
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_preview_hi1336front,
		sizeof(addr_data_pair_preview_hi1336front) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x0b04, 0x037c); //isp enable 0x037e
	//write_cmos_sensor(0x027e, 0x0100); //tg enable
#endif
}

#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_30fps_hi1336front[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	...
	0x0246, 0x0020, //y read dummy address
	0x020a, 0x0cfe, //coarse integ time
	0x021c, 0x0008, //coarse integ time short for iHDR
	0x0206, 0x05dd, //line length pck
...
kal_uint16 addr_data_pair_capture_15fps_hi1336front[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:01_isp_div2(1/2)
	0x0732, 0xe0b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:01_mipi_div2(1/2)
	0x1118, 0x0006, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
	0x1200, 0x011f, //PDPC BYPASS : Dyna-DPC ON, PDEN flag OFF, PD-DPC ON
	0x1204, 0x1c01, //PDPC DC Counting OFF, PD Around BYPASS OFF
	0x1240, 0x0100, //pdpc_pd_cnt_max_value
	...
	0x0238, 0x1111, //y even/odd inc active
	0x0246, 0x0020, //y read dummy address
	0x020a, 0x19fd, //coarse integ time
	0x021c, 0x0008, //coarse integ time short for iHDR
	0x0206, 0x05dd, //line length pck
	0x020e, 0x1a02, //frame length lines
	0x0b12, 0x1070, //x output size
	0x0b14, 0x0c30, //y output size
	0x0204, 0x0000, //d2a_row_binning_en
	0x041c, 0x0048, //pdaf patch start x-address
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_hi1336front,
		sizeof(addr_data_pair_capture_30fps_hi1336front) /
		sizeof(kal_uint16));

	} else {
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_capture_15fps_hi1336front,
		sizeof(addr_data_pair_capture_15fps_hi1336front) /
		sizeof(kal_uint16));
	}
#else
  if( currefps == 300) {
	  ...
	  write_cmos_sensor(0x0246, 0x0020); //y read dummy address
	  write_cmos_sensor(0x020a, 0x0cfe); //coarse integ time
	  write_cmos_sensor(0x021c, 0x0008); //coarse integ time short for iHDR
	  write_cmos_sensor(0x0206, 0x05dd); //line length pck
	  write_cmos_sensor(0x020e, 0x0d02); //frame length lines
...
// PIP
	  ...
	  write_cmos_sensor(0x3250, 0xa060); //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	  write_cmos_sensor(0x0730, 0x770f); //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:01_isp_div2(1/2)
	  write_cmos_sensor(0x0732, 0xe0b0); //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:01_mipi_div2(1/2)
	  write_cmos_sensor(0x1118, 0x0006); //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
	  write_cmos_sensor(0x1200, 0x011f); //PDPC BYPASS : Dyna-DPC ON, PDEN flag OFF, PD-DPC ON
	  write_cmos_sensor(0x1204, 0x1c01); //PDPC DC Counting OFF, PD Around BYPASS OFF
	  write_cmos_sensor(0x1240, 0x0100); //pdpc_pd_cnt_max_value
	  ...
	  write_cmos_sensor(0x0238, 0x1111); //y even/odd inc active
	  write_cmos_sensor(0x0246, 0x0020); //y read dummy address
	  write_cmos_sensor(0x020a, 0x19fd); //coarse integ time
	  write_cmos_sensor(0x021c, 0x0008); //coarse integ time short for iHDR
	  write_cmos_sensor(0x0206, 0x05dd); //line length pck
	  write_cmos_sensor(0x020e, 0x1a02); //frame length lines
	  write_cmos_sensor(0x0b12, 0x1070); //x output size
	  write_cmos_sensor(0x0b14, 0x0c30); //y output size
	  write_cmos_sensor(0x0204, 0x0000); //d2a_row_binning_en
	  write_cmos_sensor(0x041c, 0x0048); //pdaf patch start x-address
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_video_hi1336front[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	0x0732, 0xe0b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:00_mipi_div2(1/1)
	0x1118, 0x0006, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
...
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_video_hi1336front,
		sizeof(addr_data_pair_video_hi1336front) /
		sizeof(kal_uint16));
#else
	  write_cmos_sensor(0x3250, 0xa060); //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	  write_cmos_sensor(0x0730, 0x770f); //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
...
#endif
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_hi1336front[] = {
	0x3250, 0xa470, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	...
	0x1010, 0x0300, //mipi_vblank_delay
	...
	0x0246, 0x0020, //y read dummy address
	0x020a, 0x033b, //coarse integ time
	0x021c, 0x0008, //coarse integ time short for iHDR
	0x0206, 0x05dd, //line length pck
...
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_hs_video_hi1336front,
		sizeof(addr_data_pair_hs_video_hi1336front) /
		sizeof(kal_uint16));
#else
// setting ver5.0 None PD 640x480 120fps
	...
	write_cmos_sensor(0x1010, 0x0300); //mipi_vblank_delay
	...
	write_cmos_sensor(0x0246, 0x0020); //y read dummy address
	write_cmos_sensor(0x020a, 0x033b); //coarse integ time
	write_cmos_sensor(0x021c, 0x0008); //coarse integ time short for iHDR
	write_cmos_sensor(0x0206, 0x05dd); //line length pck
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_hi1336front[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
...
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_slim_video_hi1336front,
		sizeof(addr_data_pair_slim_video_hi1336front) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x3250, 0xa060); //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	write_cmos_sensor(0x0730, 0x770f); //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom1_24fps_hi1336front[] = {
	0x3250, 0xa060, //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
	0x0730, 0x540f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1)
	0x0732, 0xe0b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:000_mipi_div1(1/1) b1-0:00_mipi_div2(1/1)
	0x1118, 0x0006, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
//	0x1200, 0x011f, //PDPC BYPASS : Dyna-DPC ON, PDEN flag OFF, PD-DPC ON
	0x1204, 0x1c01, //PDPC DC Counting OFF, PD Around BYPASS OFF
	...
	0x1022, 0x0a31, //mipi_tclk_prepare, tclk_zero
	0x1024, 0x030b, //mipi_tclk_pre, ths_prepare
	0x1026, 0x0d0d, //mipi_ths_zero, ths_trail
	0x1028, 0x1a0a, //mipi_tclk_post, tclk_trail
	0x102a, 0x1311, //mipi_texit, tsync
	0x102c, 0x2400, //mipi_tpd_sync
	0x1010, 0x07d0, //mipi_vblank_delay
	0x1012, 0x00ab, //mipi_ch0_hblank_delay
	0x1014, 0x0001, //mipi_hblank_short_delay1
	0x1016, 0x0001, //mipi_hblank_short_delay2
	0x101a, 0x0001, //mipi_pd_hblank_delay
	0x1038, 0x4100, //mipi_virtual_channel_ctrl
	0x1042, 0x0108, //mipi_pd_sep_ctrl_h, mipi_pd_sep_ctrl_l
	0x1048, 0x0080, //mipi_pd_max_col_size
	0x1044, 0x0100, //mipi_pd_col_size
	...
	0x0238, 0x1111, //y even/odd inc active
	0x0246, 0x0020, //y read dummy address
	0x020a, 0x0cf0, //coarse integ time
	0x021c, 0x0008, //coarse integ time short for iHDR
	0x0206, 0x0754, //line length pck
	0x020e, 0x0cf4, //frame length lines
	0x0b12, 0x1070, //x output size
	0x0b14, 0x0c30, //y output size
	0x0204, 0x0000, //d2a_row_binning_en
	0x041c, 0x0048, //pdaf patch start x-address
...
#if MULTI_WRITE
	hi1336front_table_write_cmos_sensor(
		addr_data_pair_custom1_24fps_hi1336front,
		sizeof(addr_data_pair_custom1_24fps_hi1336front) /
		sizeof(kal_uint16));
#endif
}
...
#define HI1336FRONT_I2C_ADDR		0x46
static void write_cmos_sensor_byte(kal_uint32 addr, kal_uint32 para)
{
    char pu_send_cmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};

    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    iWriteRegI2C(pu_send_cmd, 3, HI1336FRONT_I2C_ADDR);
}
...
static kal_uint16 read_cmos_sensor_byte(kal_uint16 addr)
{
    ...
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    iReadRegI2C(pu_send_cmd , 2, (u8*)&get_byte, 1, HI1336FRONT_I2C_ADDR);
    return get_byte;
}
...
static kal_uint16 OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
    ...
    write_cmos_sensor_byte(0x030a, (otp_addr & 0xFF00) >> 8 ); //start address H
    write_cmos_sensor_byte(0x030b, otp_addr & 0xFF); //start address L
    write_cmos_sensor_byte(0x0302, 0x01); //single read
	//mDELAY(10);
    data = read_cmos_sensor_byte(0x0308); //OTP data read
	return data;
}

void HI1336FRONT_OTPSetting(void)
{
	LOG_INF("HI1336FRONT_OTPSetting begin:\n ");
    write_cmos_sensor_byte(0x0b02, 0x01);   // fast standby on
    write_cmos_sensor_byte(0x0809, 0x00);   // stream off
    write_cmos_sensor_byte(0x0b00, 0x00);   // stream off
    mdelay(10);
    write_cmos_sensor_byte(0x0260, 0x10);   // OTP mode display
    write_cmos_sensor_byte(0x0809, 0x01);   // stream on
    write_cmos_sensor_byte(0x0b00, 0x01);   // stream on
    mdelay(1);
	LOG_INF("HI1336FRONT_OTPSetting exit :\n ");
}

/*HI1336FRONT OTP bring up*/
static void hi1336front_otp_format_read(otp_format_t *p, enum OTP_FORM otp_form, UINT8* data_c)
{
	UINT16 start_addr = p->start_addr;
	UINT16 size = p->size;
	UINT8 *data = data_c + p->offset;
	int i;

	UINT8 group;
	UINT32 checksum_read = 0;
	UINT32 checksum = 0;

    switch(OTP_read_cmos_sensor(start_addr - 1)){
        case 0x01:
            group = 1;
            break;
        case 0x13:
	...
	start_addr += (group-1)*(size+4);
    pr_err("hi1336front_otp_format_read group(0x%x) start_addr(0x%x)", group, start_addr);
	for (i = 0; i < size; i++) {
        write_cmos_sensor_byte(0x030a, ((start_addr+i) >> 8)); //start addr H
        write_cmos_sensor_byte(0x030b, (start_addr+i));	//start addr L
        write_cmos_sensor_byte(0x0302, 0x01);	//single mode
		data[i] = read_cmos_sensor_byte(0x0308); //otp data read
		checksum += data[i];
	}

	checksum_read = (read_cmos_sensor_byte(0x0308)&0xFF) + ((read_cmos_sensor_byte(0x0308)&0xFF) << 8) + ((read_cmos_sensor_byte(0x0308)&0xFF) << 16) + ((read_cmos_sensor_byte(0x0308)&0xFF) << 24);

	data_c[(MDH50LM_OTP_SIZE - (3 - otp_form))] = ((checksum % 0x0000FFFF) == (checksum_read % 0x0000FFFF));
	printk("hi1336front_otp_format_read otp_form(%d) group(%d) checksum(0x%04x) checksum_read(0x%x) check_ok(%d)",
			otp_form, group, (checksum % 0x0000FFFF), (checksum_read % 0x0000FFFF), data_c[(MDH50LM_OTP_SIZE - (3 - otp_form))]);
#if 0 //for debug
	for (i = 0; i < size; i += 6) {
		printk("hi1336front_otp_read data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	}
	while (i < size) {
		printk("hi1336front_otp_read data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
}

extern mdh50lm_otp_data_t hi1336front_otp_data;
extern UINT8 imgsensor_model_name;
static void hi1336front_otp_read(void)
{
	UINT8 *data = (UINT8*)&hi1336front_otp_data;
	int i;
	otp_format_t otp_format_data[MDH50LM_OTP_FORM_MAX] = {
		{0x04C2, MDH50LM_AWB_SIZE    , 0},
		{0x04E1, MDH50LM_LSC_SIZE * 4, MDH50LM_AWB_SIZE},
		{0x0F4A, MDH50LM_AWB_SIZE    , MDH50LM_AWB_SIZE + (MDH50LM_LSC_SIZE * 4)},
	};

	pr_err("hi1336front_otp_read");
    HI1336FRONT_OTPSetting();
	for (i = 0; i < MDH50LM_OTP_FORM_MAX; i++)
		hi1336front_otp_format_read(&otp_format_data[i], i, data);

    imgsensor_model_name = OTP_read_cmos_sensor(0x402);

    write_cmos_sensor_byte(0x0809, 0x00);   // stream off
    write_cmos_sensor_byte(0x0b00, 0x00);   // stream off
    mdelay(10);
    write_cmos_sensor_byte(0x0260, 0x00);   // OTP mode display
    write_cmos_sensor_byte(0x0809, 0x01);   // stream on
    write_cmos_sensor_byte(0x0b00, 0x01);   // stream on
    mdelay(1);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(front_sensor_name, "hi1336front_lgit");/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/
				pr_debug("i2c write id : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);

                sensor_init();
                hi1336front_otp_read();
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		LOG_DEBUG("Read id fail,sensor id: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	LOG_DEBUG("[HI1336FRONT] open sensor\n");
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		LOG_DEBUG("open sensor id fail: 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
{
	LOG_DEBUG("[HI1336FRONT] close sensor\n");
	return ERROR_NONE;
}	/*	close  */
...
{
	LOG_DEBUG("[HI1336FRONT] preview start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[HI1336FRONT] capture start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_DEBUG("[HI1336FRONT] normal_video start\n");
	spin_lock(&imgsensor_drv_lock);
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		spin_unlock(&imgsensor_drv_lock);
			...
			LOG_DEBUG("fps %d fps not support,use cap: %d fps!\n",
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		...
		read_hi1336front_eeprom((kal_uint16)(*feature_data),
				(char *)(uintptr_t)(*(feature_data+1)),
				(kal_uint32)(*(feature_data+2)) );
		...
		switch (*feature_data) {
			...
			case MSDK_SCENARIO_ID_SLIM_VIDEO:
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
				break;
			case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0; //[LGE] PDAF is not possible in preview mode
				break;
			case MSDK_SCENARIO_ID_CUSTOM1:
			case MSDK_SCENARIO_ID_CUSTOM2:
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
...
UINT32 HI1336FRONT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	printk("HI1336FRONT_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	HI1336FRONT_MIPI_RAW_SensorInit	*/
