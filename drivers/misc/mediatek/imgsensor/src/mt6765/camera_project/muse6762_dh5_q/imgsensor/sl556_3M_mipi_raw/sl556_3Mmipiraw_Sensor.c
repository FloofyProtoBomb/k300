...
#include "sl556_3Mmipiraw_Sensor.h"

#define PFX "sl556_3M_camera_sensor"
...
static struct sl556_3M_imgsensor_info_struct imgsensor_info = {
	.sensor_id = SL556_3M_SENSOR_ID,
	.checksum_value = 0x55e2a82f,
	.pre = {
		.pclk = 167700000,				//record different mode's pclk
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 83850000,//424M x 2 / 10
	...
	.cap = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 300,
		.mipi_pixel_rate = 167700000,//848M x 2 / 10
	},
	...
	.hs_video = {
        .pclk = 167700000,
        .linelength = 2816,				//record different mode's linelength
		.framelength = 526,				//record different mode's framelength
		.startx = 0,
		.starty = 0,
		...
		.max_framerate = 1200,
		.mipi_pixel_rate = 41925000,//212M x 2 / 10
	},
    .slim_video = {
		.pclk = 176000000,
		.linelength = 2816,
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr, //SENSOR_OUTPUT_FORMAT_RAW_B,
	.mclk = 26,
...
static struct sl556_3M_imgsensor_struct imgsensor = {
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 2592, 1944, 272, 204,   2048, 1536,   2048, 1536,   0, 0, 2048, 1536,   0, 0, 2048, 1536}, 	// capture
 { 2592, 1944, 272, 204,   2048, 1536,   2048, 1536,   0, 0, 2048, 1536,   0, 0, 2048, 1536}, 	// video
...
static kal_uint16 sl556_3M_table_write_cmos_sensor(
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_sl556_3M[] = {
    0x0e00, 0x0102, 
    0x0e02, 0x0102, 
    0x0e0c, 0x0100, 
    0x2000, 0x4031, 
...
{
    LOG_DEBUG("[SL556_3M_FRONT] init setting start\n");

#if MULTI_WRITE
sl556_3M_table_write_cmos_sensor(
	addr_data_pair_init_sl556_3M,
	sizeof(addr_data_pair_init_sl556_3M) /
	sizeof(kal_uint16));
    ...
    write_cmos_sensor(0x0958, 0xaa80); 
    write_cmos_sensor(0x095a, 0x2080); 
    ...
    write_cmos_sensor(0x0006, 0x07d5); 
    ...
    write_cmos_sensor(0x0074, 0x07d3); 
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_sl556_3M[] = {
    0x0b0a, 0x8259,
    0x0f30, 0x8135,
    0x0f32, 0x7167,
    0x004a, 0x0100,
    ...
    0x0008, 0x0b00,
    0x005a, 0x0404,
    0x0012, 0x000c, //x start address
    0x0018, 0x0a33, //x end address
    0x0022, 0x0008,
    0x0028, 0x0017,
    0x0024, 0x0022,
    0x002a, 0x002b,
    0x0026, 0x0030, //y start address
    0x002c, 0x07c7, //y end address
    0x002e, 0x3311,
    0x0030, 0x3311,
    0x0032, 0x3311,
    0x0006, 0x07c0, //frame length
    0x0116, 0x03e6,
    0x0a22, 0x0000,
    0x0a12, 0x0510, //formatter column output size
    0x0a14, 0x03cc, //formatter row output size
    0x003e, 0x0000,
    0x0074, 0x07be, //the integration time control
    0x0070, 0x03df,
    0x0804, 0x0200,
    0x0a04, 0x016a,
    0x090c, 0x09c0,
...
static void preview_setting(void)
{
#if MULTI_WRITE
	sl556_3M_table_write_cmos_sensor(
		addr_data_pair_preview_sl556_3M,
		sizeof(addr_data_pair_preview_sl556_3M) /
		sizeof(kal_uint16));
    ...
    write_cmos_sensor(0x0012, 0x000c); //x start address
    write_cmos_sensor(0x0018, 0x0a33); //x end address
    write_cmos_sensor(0x0022, 0x0008);
    write_cmos_sensor(0x0028, 0x0017);
    write_cmos_sensor(0x0024, 0x0022);
    write_cmos_sensor(0x002a, 0x002b);
    write_cmos_sensor(0x0026, 0x0030); //y start address
    write_cmos_sensor(0x002c, 0x07c7); //y end address
    write_cmos_sensor(0x002e, 0x3311);
    write_cmos_sensor(0x0030, 0x3311);
    write_cmos_sensor(0x0032, 0x3311);
    write_cmos_sensor(0x0006, 0x07d5); //frame length
    write_cmos_sensor(0x0116, 0x03e6);
    write_cmos_sensor(0x0a22, 0x0000);
    write_cmos_sensor(0x0a12, 0x0510); //formatter column output size
    write_cmos_sensor(0x0a14, 0x03cc); //formatter row output size
    write_cmos_sensor(0x003e, 0x0000);
    write_cmos_sensor(0x0074, 0x07d3); //the integration time control
    write_cmos_sensor(0x0070, 0x03e9);
    write_cmos_sensor(0x0804, 0x0200);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_fps_sl556_3M[] = { //15ps
	0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
...
kal_uint16 addr_data_pair_capture_30fps_sl556_3M[] = {	//30ps
    0x0b0a, 0x8252,
	0x0f30, 0x8135,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
{
LOG_DEBUG("[SL556_3M_FRONT] capture setting start currentfps : %d\n", currefps);
#if MULTI_WRITE
	if (currefps == 300) {
		sl556_3M_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_sl556_3M,
		sizeof(addr_data_pair_capture_30fps_sl556_3M) /
		sizeof(kal_uint16));
	}else {
		sl556_3M_table_write_cmos_sensor(
		addr_data_pair_capture_fps_sl556_3M,
		sizeof(addr_data_pair_capture_fps_sl556_3M) /
		sizeof(kal_uint16));
	}
#else   
	if( currefps == 300 )
	{
        ...
        write_cmos_sensor(0x0008, 0x0b00);
        write_cmos_sensor(0x005a, 0x0202);
        write_cmos_sensor(0x0012, 0x000e); //x start address
        write_cmos_sensor(0x0018, 0x0a31); //x end address
        write_cmos_sensor(0x0022, 0x0008);
        write_cmos_sensor(0x0028, 0x0017);
        write_cmos_sensor(0x0024, 0x0028);
        write_cmos_sensor(0x002a, 0x002d);
        ...
        write_cmos_sensor(0x0116, 0x07b6);
        write_cmos_sensor(0x0a22, 0x0000);
        write_cmos_sensor(0x0a12, 0x0a20); //formatter column output size
        write_cmos_sensor(0x0a14, 0x0798); //formatter row output size
        write_cmos_sensor(0x003e, 0x0000);
        write_cmos_sensor(0x0074, 0x07d3); //the integration time control
        write_cmos_sensor(0x0070, 0x03e9);
        write_cmos_sensor(0x0804, 0x0200);
	...
	else
	{
        ...
        write_cmos_sensor(0x0030, 0x1111);
        write_cmos_sensor(0x0032, 0x1111);
        write_cmos_sensor(0x0006, 0x0fa8); //frame length
        write_cmos_sensor(0x0116, 0x07b6);
        write_cmos_sensor(0x0a22, 0x0000);
        write_cmos_sensor(0x0a12, 0x0a20); //formatter column output size
        write_cmos_sensor(0x0a14, 0x0798); //formatter row output size
        write_cmos_sensor(0x003e, 0x0000);
        write_cmos_sensor(0x0074, 0x0fa6); //the integration time control
        write_cmos_sensor(0x0070, 0x07d3);
        write_cmos_sensor(0x0804, 0x0200);
        write_cmos_sensor(0x0a04, 0x014a);
        write_cmos_sensor(0x090c, 0x0fdc);
        write_cmos_sensor(0x090e, 0x002d);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_normal_video_sl556_3M[] = {
    0x0b0a, 0x8252,
	0x0f30, 0x8135,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
#if MULTI_WRITE
	sl556_3M_table_write_cmos_sensor(
	    addr_data_pair_normal_video_sl556_3M,
	    sizeof(addr_data_pair_normal_video_sl556_3M) /
	    sizeof(kal_uint16));
...
#endif
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_sl556_3M[] = {
    0x0b0a, 0x8252,
    0x0f30, 0x8135,
    0x0f32, 0x7267,
    0x004a, 0x0100,
    ...
    0x0008, 0x0b00,
    0x005a, 0x0208,
    0x0012, 0x0018, //x start address
    0x0018, 0x0a27, //x end address
    0x0022, 0x0008,
    0x0028, 0x0017,
    0x0024, 0x002e,
    0x002a, 0x0033,
    0x0026, 0x003c, //y start address
    0x002c, 0x07bb, //y end address
    0x002e, 0x1111,
    0x0030, 0x1111,
    0x0032, 0x7711,
    0x0006, 0x020e, //frame length
    0x0116, 0x0206,
    0x0a22, 0x0100,
    0x0a12, 0x0280, //formatter column output size
    0x0a14, 0x01e0, //formatter row output size
    0x003e, 0x0000,
    0x0074, 0x020c, //the integration time control
    0x0070, 0x0107,
    0x0804, 0x0200,
    0x0a04, 0x016a,
    0x090c, 0x0270,
...
static void hs_video_setting(void)
{
#if MULTI_WRITE
	sl556_3M_table_write_cmos_sensor(
		addr_data_pair_hs_video_sl556_3M,
		sizeof(addr_data_pair_hs_video_sl556_3M) /
		sizeof(kal_uint16));
#else
    write_cmos_sensor(0x0b0a, 0x8252);
    write_cmos_sensor(0x0f30, 0x6a25);
    ...
    write_cmos_sensor(0x0012, 0x0018); //x start address
    write_cmos_sensor(0x0018, 0x0a27); //x end address
    write_cmos_sensor(0x0022, 0x0008);
    write_cmos_sensor(0x0028, 0x0017);
    write_cmos_sensor(0x0024, 0x002e);
    write_cmos_sensor(0x002a, 0x0033);
    write_cmos_sensor(0x0026, 0x003c); //y start address
    write_cmos_sensor(0x002c, 0x07bb); //y end address
    write_cmos_sensor(0x002e, 0x1111);
    write_cmos_sensor(0x0030, 0x1111);
    write_cmos_sensor(0x0032, 0x7711);
    write_cmos_sensor(0x0006, 0x021e); //frame length
    write_cmos_sensor(0x0116, 0x07b6);
    write_cmos_sensor(0x0a22, 0x0100);
    write_cmos_sensor(0x0a12, 0x0280); //formatter column output size
    write_cmos_sensor(0x0a14, 0x01e0); //formatter row output size
    write_cmos_sensor(0x003e, 0x0000);
    write_cmos_sensor(0x0074, 0x021c); //the integration time control
    write_cmos_sensor(0x0070, 0x010e);
    write_cmos_sensor(0x0804, 0x0200);
    write_cmos_sensor(0x0a04, 0x016a);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_sl556_3M[] = {
    //0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
...
#if MULTI_WRITE
	sl556_3M_table_write_cmos_sensor(
		addr_data_pair_slim_video_sl556_3M,
		sizeof(addr_data_pair_slim_video_sl556_3M) /
		sizeof(kal_uint16));
#else
...
#define SL556_I2C_ADDR		0x40
static void write_cmos_sensor_byte(kal_uint32 addr, kal_uint32 para)
{
    char pu_send_cmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};

    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    iWriteRegI2C(pu_send_cmd, 3, SL556_I2C_ADDR);
}
...
static kal_uint16 read_cmos_sensor_byte(kal_uint16 addr)
{
    ...
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    iReadRegI2C(pu_send_cmd , 2, (u8*)&get_byte, 1, SL556_I2C_ADDR);
    return get_byte;
}
...
static kal_uint16 OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
    ...
    write_cmos_sensor_byte(0x010a, (otp_addr & 0xFF00) >> 8 ); //start addr H
    write_cmos_sensor_byte(0x010b, otp_addr & 0xFF);	//start addr L
    write_cmos_sensor_byte(0x0102, 0x01);	//single mode
	//mDELAY(10);
    data = read_cmos_sensor_byte(0x0108); //OTP data read
	return data;
}

void SL556_3M_OTPSetting(void)
{
	LOG_INF("SL556_3M_OTPSetting begin:\n ");
	...
	write_cmos_sensor_byte(0x0D04, 0x01); //OTP mode enable
	write_cmos_sensor_byte(0x0D02, 0x07); //OTP mode output drivability
	write_cmos_sensor_byte(0x003E, 0x10); //OTP R/W
	write_cmos_sensor_byte(0x0a00, 0x01); //sleep off
	mDELAY(1);
	LOG_INF("SL556_3M_OTPSetting exit :\n ");
}

/*SL556 OTP bring up*/
static void sl556_3M_otp_format_read(otp_format_t *p, enum DH5LM_OTP_FORM otp_form, UINT8* data_c)
{
	...
	start_addr += (group-1)*(size+2);
    pr_err("sl556_3M_otp_format_read group(0x%x) start_addr(0x%x)", group, start_addr);

	for (i = 0; i < size; i++) {
        write_cmos_sensor_byte(0x010a, ((start_addr+i) >> 8)); //start addr H
        write_cmos_sensor_byte(0x010b, (start_addr+i));	//start addr L
        write_cmos_sensor_byte(0x0102, 0x01);	//single mode

		data[i] = read_cmos_sensor_byte(0x108); //otp data read
        if (size == MDH5LM_AWB_SIZE){
            if (i % 2 == 0){
                checksum += data[i] * 256;
            } else {
                checksum += data[i];
            }
        } else if (size == (MDH5LM_LSC_SIZE*4) ){
            checksum += data[i];
        }
	}

	checksum_read = ((read_cmos_sensor_byte(0x0108)&0xFF) << 8) + (read_cmos_sensor_byte(0x0108)&0xFF);

	data_c[(MDH5LM_OTP_SIZE - (4 - otp_form))] = ((checksum % 0xFFFF) == (checksum_read % 0xFFFF));
	printk("sl556_3M_otp_format_read otp_form(%d) group(%d) checksum(0x%04x) checksum_read(0x%04x) check_index(%d) check_ok(%d)",
			otp_form, group, checksum % 0xFFFF, checksum_read % 0xFFFF, (MDH5LM_OTP_SIZE - (4 - otp_form)), data_c[(MDH5LM_OTP_SIZE - (4 - otp_form))]);
#if 0 // for debug
	for (i = 0; i < size; i += 6) {
		printk("sl556_3M_otp_read data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	}
	while (i < size) {
		printk("sl556_3M_otp_read data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
}

extern mdh5lm_otp_data_t sl556_3M_otp_data;
extern UINT8 imgsensor_model_name;
static void sl556_3M_otp_read(void)
{
	UINT8 *data = (UINT8*)&sl556_3M_otp_data;
	int i;
	otp_format_t otp_format_data[MDH5LM_OTP_FORM_MAX] = {
		//{0x0000, 100    , 0},
		{0x0401, MDH5LM_AWB_SIZE    , 0},
		{0x041A, MDH5LM_AWB_SIZE    , MDH5LM_AWB_SIZE},
		{0x0433, MDH5LM_LSC_SIZE * 4, MDH5LM_AWB_SIZE + MDH5LM_AWB_SIZE},
		{0x0E96, MDH5LM_LSC_SIZE * 4, MDH5LM_AWB_SIZE + MDH5LM_AWB_SIZE + (MDH5LM_LSC_SIZE * 4)},
	};

	pr_err("sl556_3M_otp_read");
    SL556_3M_OTPSetting();
	for (i = 0; i < MDH5LM_OTP_FORM_MAX; i++)
		sl556_3M_otp_format_read(&otp_format_data[i], i, data);

    imgsensor_model_name = OTP_read_cmos_sensor(0x18F9);

	write_cmos_sensor_byte(0x0a00, 0x00); //stand by on
	write_cmos_sensor_byte(0x004a, 0x00); //TG off
	mdelay(10);
    write_cmos_sensor_byte(0x0d04, 0x00); //OTP mode disable
    write_cmos_sensor_byte(0x003e, 0x00); //OTP R/W mode
    write_cmos_sensor_byte(0x004a, 0x01); //TG on
			...
			do {
				*sensor_id =return_sensor_id()+2;
				if (*sensor_id == imgsensor_info.sensor_id) {				
                    strcpy(front_sensor_name, "sl556_3M_front_lgit");/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/
...
static kal_uint32 open(void)
{
	...
	LOG_DEBUG("[open]: PLATFORM:MT6762,MIPI 24LANE\n");
	LOG_DEBUG("preview 1296*972@30fps,360Mbps/lane;"
		"capture 2592*1944@30fps,880Mbps/lane\n");
    LOG_DEBUG("[SL556_3M_FRONT] open sensor\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
...
{
    LOG_DEBUG("[SL556_3M_FRONT] close sensor\n");
	/*No Need to implement this function*/ 
	return ERROR_NONE;
}	/*	close  */
...
{
    LOG_DEBUG("[SL556_3M_FRONT] preview start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[SL556FRONT] capture start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_DEBUG("[SL556FRONT] normal_video start\n");
...
UINT32 SL556_3M_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
    printk("SL556_3M_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	SL556_3M_MIPI_RAW_SensorInit	*/
