...
#include "sl556mipiraw_Sensor.h" 

#define PFX "sl556_camera_sensor"
...
extern char g_lge_product_name[16];
int CRK_init_setting = 0;

static struct sl556_imgsensor_info_struct imgsensor_info = {
	.sensor_id = SL556_SENSOR_ID,
...
static struct sl556_imgsensor_struct imgsensor = {
...
static kal_uint16 sl556_table_write_cmos_sensor(
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_sl556[] = {
    0x0e00, 0x0102, 
    0x0e02, 0x0102, 
    0x0e0c, 0x0100, 
    0x2000, 0x4031, 
...
{
LOG_DEBUG("[SL556FRONT] init setting start\n");
LOG_DEBUG("[g_lge_product_name] %s\n", g_lge_product_name);

if(!strcmp(g_lge_product_name, "mdh5lm_3mg")){
	LOG_DEBUG("[lge_product_name] %s\n", g_lge_product_name);
	CRK_init_setting = 1;
}
else{
	LOG_DEBUG("[lge_product_name] %s\n", g_lge_product_name);
...
#if MULTI_WRITE
sl556_table_write_cmos_sensor(
	addr_data_pair_init_sl556,
	sizeof(addr_data_pair_init_sl556) /
	sizeof(kal_uint16));
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_sl556[] = {
    0x0b0a, 0x8259,
    0x0f30, 0x8135,
    0x0f32, 0x7167,
    0x004a, 0x0100,
...
#if MULTI_WRITE
	sl556_table_write_cmos_sensor(
		addr_data_pair_preview_sl556,
		sizeof(addr_data_pair_preview_sl556) /
		sizeof(kal_uint16));
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_fps_sl556[] = { 
    0x0b0a, 0x8252,
    0x0f30, 0x8135,
    0x0f32, 0x7067,
    0x004a, 0x0100,
    ...
    0x0008, 0x0b00,
    0x005a, 0x0202,
    0x0012, 0x000e, //x start address
    0x0018, 0x0a31, //x end address
    0x0022, 0x0008,
    0x0028, 0x0017,
    0x0024, 0x0028,
    0x002a, 0x002d,
    ...
    0x0030, 0x1111,
    0x0032, 0x1111,
    0x0006, 0x0f80, //frame length
    0x0116, 0x07b6,
    0x0a22, 0x0000,
    0x0a12, 0x0a20, //formatter column output size
    0x0a14, 0x0798, //formatter row output size
    0x003e, 0x0000,
    0x0074, 0x0f7e, //the integration time control
    0x0070, 0x07c0,
    0x0804, 0x0200,
    0x0a04, 0x014a,
    0x090c, 0x0fdc,
...
kal_uint16 addr_data_pair_capture_30fps_sl556[] = {	//30ps
    0x0b0a, 0x8252,
    0x0f30, 0x8135,
    0x0f32, 0x7067,
    0x004a, 0x0100,
...
{
LOG_DEBUG("[SL556FRONT] capture setting start currentfps : %d\n", currefps);
#if MULTI_WRITE
	if (currefps == 300) {
		sl556_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_sl556,
		sizeof(addr_data_pair_capture_30fps_sl556) /
		sizeof(kal_uint16));
    }
	else{
		sl556_table_write_cmos_sensor(
		addr_data_pair_capture_fps_sl556,
		sizeof(addr_data_pair_capture_fps_sl556) /
		sizeof(kal_uint16));
	}
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_normal_video_sl556[] = {
    0x0b0a, 0x8252,
    0x0f30, 0x8135,
    0x0f32, 0x7067,
    0x004a, 0x0100,
...
#if MULTI_WRITE
    sl556_table_write_cmos_sensor(
		addr_data_pair_normal_video_sl556,
		sizeof(addr_data_pair_normal_video_sl556) /
		sizeof(kal_uint16));
...
#endif
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_sl556[] = {
    0x0b0a, 0x8252,
    0x0f30, 0x8135,
    0x0f32, 0x7267,
    0x004a, 0x0100,
...
#if MULTI_WRITE
	sl556_table_write_cmos_sensor(
		addr_data_pair_hs_video_sl556,
		sizeof(addr_data_pair_hs_video_sl556) /
		sizeof(kal_uint16));
#else
    write_cmos_sensor(0x0b0a, 0x8252);
    write_cmos_sensor(0x0f30, 0x6a25);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_sl556[] = {
    //0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
...
#if MULTI_WRITE
	sl556_table_write_cmos_sensor(
		addr_data_pair_slim_video_sl556,
		sizeof(addr_data_pair_slim_video_sl556) /
		sizeof(kal_uint16));
#else
...
void SL556_OTPSetting(void)
{
	LOG_INF("SL556_OTPSetting begin:\n ");
	...
	mDELAY(1);
	LOG_INF("SL556_OTPSetting exit :\n ");
}

/*SL556 OTP bring up*/
static void sl556_otp_format_read(otp_format_t *p, enum DH5LM_OTP_FORM otp_form, UINT8* data_c)
{
    ...
    pr_err("sl556_otp_format_read group(0x%x) start_addr(0x%x)", group, start_addr);
	...
	printk("sl556_otp_format_read otp_form(%d) group(%d) checksum(0x%04x) checksum_read(0x%04x) check_index(%d) check_ok(%d)",
			otp_form, group, checksum % 0xFFFF, checksum_read % 0xFFFF, (MDH5LM_OTP_SIZE - (4 - otp_form)), data_c[(MDH5LM_OTP_SIZE - (4 - otp_form))]);
#if 0 // for debug
	for (i = 0; i < size; i += 6) {
		printk("sl556_otp_read data %d: 0x%02x, 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", i, data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]);
	}
	while (i < size) {
		printk("sl556_otp_read data %d: 0x%02x ", i, data[i]);
		i++;
	}
#endif
}

extern mdh5lm_otp_data_t sl556_otp_data;
extern UINT8 imgsensor_model_name;
static void sl556_otp_read(void)
{
	UINT8 *data = (UINT8*)&sl556_otp_data;
	int i;
	...
	pr_err("sl556_otp_read");
    SL556_OTPSetting();
	for (i = 0; i < MDH5LM_OTP_FORM_MAX; i++)
		sl556_otp_format_read(&otp_format_data[i], i, data);
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
                    strcpy(front_sensor_name, "sl556front_lgit");/*LGE_CHANGE, 2019-11-14, add the camera identifying logic , bk.bae@lge.com*/
...
static kal_uint32 open(void)
{
    ...
    LOG_DEBUG("[SL556FRONT] open sensor\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
...
{
    LOG_DEBUG("[SL556FRONT] close sensor\n");
	/*No Need to implement this function*/ 
	return ERROR_NONE;
}	/*	close  */
...
{
    LOG_DEBUG("[SL556FRONT] preview start\n");
	spin_lock(&imgsensor_drv_lock);
...
UINT32 SL556_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    ...
    printk("SL556_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
