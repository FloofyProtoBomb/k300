...
/* PD_TIMER (out of spec ) */
	"PD_TIMER_VSAFE0V_DELAY_TRAINSITION",
...
#define PD_TIMER_VSAFE0V_DLY_TOUT		300
#else
...
DECL_TCPC_TIMEOUT_RANGE(PD_TIMER_VDM_RESPONSE, 28, 30),
...
/* PD_TIMER (out of spec) */
DECL_TCPC_TIMEOUT(PD_TIMER_VSAFE0V_DELAY_TRAINSITION, 50),
		...
		struct tcpc_device *tcpc_dev, uint32_t timer_id)
{
	...
	case PD_TIMER_UVDM_RESPONSE:
		...
		break;
	case PD_TIMER_VSAFE0V_DELAY_TRAINSITION:
...
/* PD_TIMER (out of spec )*/
static enum hrtimer_restart tcpc_timer_vsafe0v_delay_trainsition(struct hrtimer *timer)
{
	int index = PD_TIMER_VSAFE0V_DELAY_TRAINSITION;
	struct tcpc_device *tcpc_dev =
		container_of(timer, struct tcpc_device, tcpc_timer[index]);
...
/* PD_TIMER (out of spec )*/
	tcpc_timer_vsafe0v_delay_trainsition,
