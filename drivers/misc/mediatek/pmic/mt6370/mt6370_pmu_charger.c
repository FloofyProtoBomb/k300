...
#ifdef CONFIG_LGE_PM
static bool ship_mode_enabled = false;
static bool is_te_enabled = false;

bool mt6370_pmu_ship_mode_enabled(void)
{
	return ship_mode_enabled;
}
EXPORT_SYMBOL(mt6370_pmu_ship_mode_enabled);

bool mt6370_pmu_is_te_enabled(void)
{
	return is_te_enabled;
}
EXPORT_SYMBOL(mt6370_pmu_is_te_enabled);
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_HZ
	bool input_suspend;
#endif
#ifdef CONFIG_LGE_PM
	bool in_aicl_meas;
...
#ifdef CONFIG_LGE_PM
static int mt6370_set_vrech(struct mt6370_pmu_charger_data *chg_data,
	u32 uV)
{
	int ret = 0;
	u8 reg_vrech = 0;

	reg_vrech = mt6370_find_closest_reg_value(
		MT6370_VRECHG_MIN,
		MT6370_VRECHG_MAX,
		MT6370_VRECHG_STEP,
		MT6370_VRECHG_NUM,
	...
	dev_info(chg_data->dev, "%s: vrech = %d (0x%02X)\n", __func__, uV,
		reg_vrech);
	...
	ret = mt6370_pmu_reg_update_bits(
		...
		MT6370_PMU_REG_CHGCTRL11,
		MT6370_MASK_VRECHG,
		reg_vrech << MT6370_SHIFT_VRECHG
	);

	return ret;
}

static int mt6370_get_vrech(struct mt6370_pmu_charger_data *chg_data,
	u32 *vrech)
{
	int ret = 0;
	u8 reg_vrech = 0;
	...
	reg_vrech = (ret & MT6370_MASK_VRECHG) >> MT6370_SHIFT_VRECHG;
	*vrech = mt6370_find_closest_real_value(MT6370_VRECHG_MIN, MT6370_VRECHG_MAX,
		MT6370_VRECHG_STEP, reg_vrech);
...
#ifdef CONFIG_LGE_PM
static int mt6370_ops_enable_hz(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

#ifdef CONFIG_LGE_PM_CHARGER_HZ
	chg_data->input_suspend = en;
#endif

	return mt6370_enable_hz(chg_data, en);
}

static int mt6370_is_hz_enabled(struct charger_device *chg_dev, bool *en)
{
	int ret = 0;
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	ret = mt6370_pmu_reg_test_bit(chg_data->chip, MT6370_PMU_REG_CHGCTRL1,
		MT6370_SHIFT_HZ_EN, en);
...
static int mt6370_set_recharge_voltage(struct charger_device *chg_dev, u32 uV)
{
	...
	return mt6370_set_vrech(chg_data, uV);
}

static int mt6370_get_recharge_voltage(struct charger_device *chg_dev, u32 *uV)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	return mt6370_get_vrech(chg_data, uV);
}

static int mt6370_enable_ship_mode(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	if (ship_mode_enabled == en)
		return 0;

	ship_mode_enabled = en;

	dev_info(chg_data->dev, "ship mode %s\n", en ? "enabled" : "disabled");

	return 0;
}

static int mt6370_is_ship_mode_enabled(struct charger_device *chg_dev, bool *en)
{
	*en = ship_mode_enabled;
...
#ifdef CONFIG_LGE_PM
	/* do not read IBUS during aicl measurement */
	if (chg_data->in_aicl_meas) {
		dev_err(chg_data->dev, "%s: measuring aicl. ignore irq\n",
			__func__);
		goto out;
	}
#endif
...
#ifdef CONFIG_LGE_PM
	/* notify if aicl not measured yet */
	if (chg_data->aicr_limit == -1) {
		dev_info(chg_data->dev, "%s: notify mivr\n", __func__);
		charger_dev_notify(chg_data->chg_dev,
				CHARGER_DEV_NOTIFY_VBUS_MIVR);
	}
#endif
...
static irqreturn_t mt6370_pmu_pwr_rdy_irq_handler(int irq, void *data)
{
	...
	dev_info(chg_data->dev, "%s\n", __func__);
#ifdef CONFIG_LGE_PM_CHARGER_HZ
	if (chg_data->input_suspend)
		mt6370_enable_hz(chg_data, true);
#endif
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_LGE_PM
	if (of_property_read_u32(np, "vrech", &chg_desc->vrech) < 0)
		dev_err(chg_data->dev, "%s: no vrech\n", __func__);
#endif
...
static int mt6370_chg_init_setting(struct mt6370_pmu_charger_data *chg_data)
{
...
#ifdef CONFIG_LGE_PM
	/* preserve ichg, aicr value set in lk */
#else /* MediaTek */
	...
	/*
	 * do not set te here for no-battery booting.
	 * charging algorithm will set this.
	 */
#else
...
#ifdef CONFIG_LGE_PM
	ret = mt6370_set_vrech(chg_data, chg_desc->vrech);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set vrech failed\n", __func__);
#endif
	...
	ret = mt6370_pmu_reg_update_bits(
		chg_data->chip, MT6370_PMU_REG_CHGHIDDENCTRL15,
		MT6370_MASK_dADC_IBIAS_SEL, MT6370_MASK_dADC_IBIAS_SEL);
	if (ret < 0)
		dev_err(chg_data->dev,
			"%s: modify ADC bias current fail\n", __func__);

	ret = mt6370_enable_hidden_mode(chg_data, false);
	if (ret < 0)
		dev_err(chg_data->dev,
			"%s: exit hidden mode failed\n", __func__);
...
#ifdef CONFIG_LGE_PM
	/* Input Control */
	.enable_hz = mt6370_ops_enable_hz,
	.is_hz_enabled = mt6370_is_hz_enabled,

	.set_recharge_voltage = mt6370_set_recharge_voltage,
	.get_recharge_voltage = mt6370_get_recharge_voltage,

	.enable_ship_mode = mt6370_enable_ship_mode,
	.is_ship_mode_enabled = mt6370_is_ship_mode_enabled,
#endif
};
...
#ifdef CONFIG_LGE_PM
static void mt6370_pmu_charger_shutdown(struct platform_device *pdev)
{
	const char shipping_mode_magic[] = "5526789";

	if (ship_mode_enabled) {
		shipping_mode_store(&pdev->dev, NULL, shipping_mode_magic,
				strlen(shipping_mode_magic));
	}
}
#endif
...
#ifdef CONFIG_LGE_PM
	.shutdown = mt6370_pmu_charger_shutdown,
#endif
