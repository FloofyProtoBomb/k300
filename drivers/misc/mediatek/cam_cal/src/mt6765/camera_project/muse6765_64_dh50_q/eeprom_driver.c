	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* Default EEPROM Slave Address Main= 0xb0 */
...
#endif
{
    ...
    int index = 0;
    mdh50lm_otp_data_t* phi1336front_otp_data = NULL;
	...
	case CAM_CALIOC_G_READ:
		pr_err("CAM_CALIOC_G_READ start! offset=%d, length=%d\n",
			ptempbuf->u4Offset, ptempbuf->u4Length);
        ...
        if(ptempbuf->sensorID == 0x1337){
            phi1336front_otp_data = &hi1336front_otp_data;

            i4RetValue = ptempbuf->u4Length;

            switch(ptempbuf->u4Offset){
                case 0x00000402:
                    *pu1Params = imgsensor_model_name;
                    break;

                case MDH50LM_AWB_5100K_FLAG:
                    *pu1Params = phi1336front_otp_data->awb_5100k_flag;
                    break;
                case MDH50LM_LSC_5100K_FLAG:
                    *pu1Params = phi1336front_otp_data->lsc_5100k_flag;
                    break;
                case MDH50LM_AWB_3100K_FLAG:
                    *pu1Params = phi1336front_otp_data->awb_3100k_flag;
                    break;

                case MDH50LM_AWB_5100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->awb_5100k.data[index];
                    }
                    break;
                case MDH50LM_LSC_5100K_R_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->lsc_5100k_r.data[index];
                    }
                    break;
                case MDH50LM_LSC_5100K_GR_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->lsc_5100k_gr.data[index];
                    }
                    break;
                case MDH50LM_LSC_5100K_GB_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->lsc_5100k_gb.data[index];
                    }
                    break;
                case MDH50LM_LSC_5100K_B_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->lsc_5100k_b.data[index];
                    }
                    break;
                case MDH50LM_AWB_3100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = phi1336front_otp_data->awb_3100k.data[index];
                    }
                    break;

                default:
                    pr_err("No menu HI1336FRONT OTP:bit error");
                    i4RetValue = 0;
                    //i4RetValue = -EPERM;
                    break;
            }
