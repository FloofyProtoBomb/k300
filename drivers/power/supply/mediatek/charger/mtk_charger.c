...
#ifdef CONFIG_LGE_PM
struct wakeup_source cable_out_notify_wakelock;
#endif

#if defined(CONFIG_LGE_TOUCH_CORE) || defined(CONFIG_LGE_TOUCH_CORE_POS)
...
void BATTERY_SetUSBState(int usb_state_value)
{
	if (is_disable_charger()) {
		...
		if ((usb_state_value < USB_SUSPEND) ||
			...
			if (pinfo->usb_state == usb_state_value)
				return;
...
#ifdef CONFIG_LGE_PM_USB_ID
	if (usb_state_value == USB_CONFIGURED)
		lge_cable_id_set_usb_configured(true);
#endif
}
...
#ifdef CONFIG_LGE_PM
int charger_manager_enable_hz(struct charger_consumer *consumer, bool en)
{
	struct charger_manager *info = consumer->cm;
	struct list_head *pos;
	struct list_head *phead = &consumer_head;
	...
	if (en != consumer->hz_disabled)
		return 0;
	consumer->hz_disabled = !en;

	mutex_lock(&consumer_mutex);
	list_for_each(pos, phead) {
		ptr = container_of(pos, struct charger_consumer, list);
		if (ptr->hz_disabled == true) {
			info->enable_hz = false;
			break;
		}
		if (list_is_last(pos, phead))
			info->enable_hz = true;
	...
	pr_info("%s: user: %s, en = %d\n", __func__,
		dev_name(consumer->dev), info->enable_hz);

	return charger_dev_enable_hz(chg_dev, info->enable_hz);
}
EXPORT_SYMBOL(charger_manager_enable_hz);
#endif
...
#ifdef CONFIG_LGE_PM
	__pm_stay_awake(&cable_out_notify_wakelock);
#endif
...
#ifdef CONFIG_LGE_PM
	__pm_wakeup_event(&cable_out_notify_wakelock, msecs_to_jiffies(3000));
	chr_err("Cable_out_notify wakelock release 3sec\n");
#endif
...
{
#ifdef CONFIG_LGE_PM
	/* check vbus depends on PMIC or Charger IC */
#else /* MediaTek */
	int vchr = 0;
...
#ifdef CONFIG_LGE_PM
static int mtk_chg_en_ship_mode_show(struct seq_file *m, void *data)
{
	struct charger_manager *pinfo = m->private;
...
static ssize_t mtk_chg_en_ship_mode_write(struct file *file,
	const char *buffer, size_t count, loff_t *data)
{
	int len = 0, ret = 0;
	char desc[32];
	...
	ret = kstrtou32(desc, 10, &enable);
	if (ret == 0) {
		charger_dev_enable_ship_mode(info->chg1_dev, enable);
		pr_info("%s: enable ship mode = %d\n", __func__, enable);

		return count;
	}

	chr_err("bad argument, echo [enable] > en_ship_mode\n");
	return count;
}
#endif
...
#ifdef CONFIG_LGE_PM
	proc_create_data("en_ship_mode", 0644, battery_dir,
			&mtk_chg_en_ship_mode_fops, info);
#endif
	...
	void *val)
{
	...
	case MTK_PD_ADAPTER:
		...
		case MTK_TYPEC_HRESET_STATUS:
			...
			break;
#ifdef CONFIG_LGE_PM
		case MTK_PD_CONNECT_RPLV_CHANGE:
			chr_err("RpLvl Change Notify\n");
			mutex_lock(&pinfo->charger_pd_lock);
			pinfo->pd_type = MTK_PD_CONNECT_RPLV_CHANGE;
			mutex_unlock(&pinfo->charger_pd_lock);
			_wake_up_charger(pinfo);
			break;
#endif
...
static int mtk_charger_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_LGE_PM
	wakeup_source_init(&cable_out_notify_wakelock, "cable out notify wakelock");
#endif
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	info->chgctrl.input_current_limit = -1;
	info->chgctrl.charging_current_limit = -1;
	info->chgctrl.constant_voltage = -1;
	info->chgctrl.input_current_boost = -1;
	info->chgctrl.input_suspend = false;
	info->chgctrl.max_charger_voltage = -1;
#endif
	...
	mutex_unlock(&consumer_mutex);
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	chgctrl_get_min_icl(&info->chgctrl.input_current_boost);
	chgctrl_get_max_icl(&info->chgctrl.input_current_limit);
	chgctrl_get_max_fcc(&info->chgctrl.charging_current_limit);
	chgctrl_get_max_vfloat(&info->chgctrl.constant_voltage);
	chgctrl_get_input_suspend(&info->chgctrl.input_suspend);
	chgctrl_get_max_vbus(&info->chgctrl.max_charger_voltage);
	if (info->chgctrl.max_charger_voltage != - 1) {
		if (info->chgctrl.max_charger_voltage < 5500000)
			info->enable_hv_charging = false;
	}
#endif
	info->chg1_consumer =
